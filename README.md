...
- 目前是「完全公開、任何人都能用」，因此請留意 Gemini API 的用量與費用；若之後要加使用限制（例如簡單密碼保護），可以在 `app/api/chat/route.js` 最前面加一段驗證邏輯。
